#include<Foundation/MemoryPool/Cache/ThreadCache.h>
namespace DM
{
	...
	{
		//获取合适的内存链表索引
		memorySize = memorySize == 0 ? ALIGNMENT : memorySize;
		if (memorySize > MAXBLOCKBYTE)return std::malloc(memorySize);

		size_t listIndex = ConvertSizeToIndex(memorySize);
		
		if (void* ptr = _FreeList[listIndex])//从列表中取一块空闲内存
		{
			void* newHead = nullptr;
			if (_FreeListSize[listIndex] > 1)
			{
				newHead = *reinterpret_cast<void**>(ptr);
			}
			--_FreeListSize[listIndex];
			//重新定位头结点
			_FreeList[listIndex] = newHead;
			return ptr;
		}
		return FetchFromCentralCache(listIndex);
	}
	...
	{
		size_t memorySize = (index + 1) * ALIGNMENT;
		// 根据对象内存大小计算批量获取的数量
		size_t batchNum = GetBatchNum(memorySize);
		// 从中心缓存批量获取空闲内存块
	...
	/// <summary>
	/// 批量获取内存块策略，size<=1024时获取的总内存不会超过2kb,size>1024时只获取一块
	/// </summary>
