...
namespace DM::RHI
{
	VulkanVertexBuffer::VulkanVertexBuffer(VulkanDevice* device, const RHIVertexBufferDesc& desc):VulkanBuffer(device)
	{
		m_Size = desc.Vertices.m_SizeBytes;
		...
		memcpy(temporaryMapedPtr, desc.Vertices.Data, (size_t)m_Size);//写入数据
		vkUnmapMemory(m_Device->GetvkDevice(), temporaryvkMemory);//解除映射
		temporaryMapedPtr = nullptr;

		device->CreatevkBuffer(m_Size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,  m_vkBuffer,  m_vkMemory);
